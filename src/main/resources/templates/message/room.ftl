...
<div class="container" id="app" v-cloak>
    ...
    <!-- 채팅방 목록 -->
    <div v-for="item in chatrooms" v-bind:key="item.chatRoomId" class="chatroom-card" @click="enterRoom(item.chatRoomId, item.memberId)" @contextmenu.prevent="showContextMenu($event, item.chatRoomId)">
        <div class="chatroom-header">
            <h6>{{ item.nickname }} </h6> <!-- 수정된 부분 -->
        </div>
        <div class="chatroom-body">
            <p>최근 메시지: {{ item.latestMessageContent || '최근 메시지가 없습니다.' }}</p> <!-- 수정된 부분 -->
        </div>
        <div class="chatroom-footer">
            <small>최근 활동: {{ item.latestMessageDate || 'N/A' }}</small> <!-- 수정된 부분 -->
        </div>
    </div>
